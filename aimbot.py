...
class Aimbot:
    ...
    def __init__(self,time_delay=0.01,mouse_value=0) -> None:
        global model
        model = YOLO()
    ...
    def main(self):
        ...
        while True:
            if win32api.GetAsyncKeyState(0x02) != self.mouse_value:
                ...
                results = model(img_bgr,verbose=False)#verbose=False)
                ...
                if boxes is not None and len(boxes) > 0:
                    for box, conf_box in zip(boxes,conf) :
                        if conf_box > 0.65:
                            x1, y1, x2, y2 = box.astype(int) # numpyを直す
